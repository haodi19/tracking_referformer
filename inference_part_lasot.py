...
def main(args):
    ...
    output_path = "/ssd1/luojingnan/LaSOT_Referformer_output_with_interval/valid_images"
...
def sub_processor(lock, pid, args, save_path_prefix, save_visualize_path_prefix, video_list, data_root_path):
    ...
    for sub_path in video_list:
        ...
        frame_path.sort()
        frame_path = [frame for i, frame in enumerate(frame_path) if i % frame_interval == 0]
        video_len = len(frame_path)
        print(video_len)
        # store images
